...
ANDROID
	...
	/* Load the CAs we trust */
	{
...
#else
		{
			...
			if (stat(eXosip_tls_ctx_params.root_ca_cert, &fileStat) != 0) {
				OSIP_TRACE(osip_trace
					(__FILE__, __LINE__, OSIP_ERROR, NULL,"stat of %s failed: %s",eXosip_tls_ctx_params.root_ca_cert,strerror(errno)));
			} else {
