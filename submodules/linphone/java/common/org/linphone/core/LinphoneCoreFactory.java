...
abstract public class LinphoneCoreFactory {
	...
	private static String factoryName = "com.sipphone.core.CoreFactory";
	...
	public static final synchronized LinphoneCoreFactory instance() {
		try {
			if (theLinphoneCoreFactory == null) {
				...
				theLinphoneCoreFactory = (LinphoneCoreFactory) lFactoryClass
						.newInstance();
			}
		} catch (Exception e) {
	...
	abstract public LinphoneAuthInfo createAuthInfo(String username,
			String password, String realm);

	abstract public LinphoneCore createLinphoneCore(
			LinphoneCoreListener listener, String userConfig,
			String factoryConfig, Object userdata) throws LinphoneCoreException;

	abstract public LinphoneCore createLinphoneCore(
			LinphoneCoreListener listener) throws LinphoneCoreException;
	 ...
	 * @return
	 */
	abstract public LinphoneAddress createLinphoneAddress(String username,
	 ...
	 * 
	 * @param address
	 *            should be like sip:joe@sip.linphone.org
	 * @return
	 */
	abstract public LinphoneAddress createLinphoneAddress(String address);

	abstract public LinphoneProxyConfig createProxyConfig(String identity,
	...
	/**
	 * Create a LinphoneFriend, similar to {@link #createLinphoneFriend()} +
	 * {@link LinphoneFriend#setAddress(LinphoneAddress)}
	 * 
	 * @param friendUri
	 *            a buddy address, must be a sip uri like
