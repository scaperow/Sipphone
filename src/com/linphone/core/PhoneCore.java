...
class PhoneCore implements LinphoneCore {
	...
	PhoneCore(LinphoneCoreListener listener, File userConfig,File factoryConfig,Object  userdata) throws IOException {
		mListener=listener;
		nativePtr = newLinphoneCore(listener,userConfig.getCanonicalPath(),factoryConfig.getCanonicalPath(),userdata);
	}
	PhoneCore(LinphoneCoreListener listener) throws IOException {
		mListener=listener;
	...
	public synchronized void addAuthInfo(LinphoneAuthInfo info) {
		isValid();
		addAuthInfo(nativePtr,((Autheration)info).nativePtr);
	}
	...
	public synchronized LinphoneProxyConfig getDefaultProxyConfig() {
		...
		if (lNativePtr!=0) {
			return new ProxyConfig(lNativePtr); 
		} else {
			return null;
		}
	}
	...
	public synchronized void setDefaultProxyConfig(LinphoneProxyConfig proxyCfg) {
		isValid();
		setDefaultProxyConfig(nativePtr,((ProxyConfig)proxyCfg).nativePtr);
	}
	public synchronized void addProxyConfig(LinphoneProxyConfig proxyCfg) throws LinphoneCoreException{
		isValid();
		if (addProxyConfig(proxyCfg,nativePtr,((ProxyConfig)proxyCfg).nativePtr) !=0) {
	...
	public synchronized void terminateCall(LinphoneCall aCall) {
		isValid();
		if (aCall!=null)terminateCall(nativePtr,((Callline)aCall).nativePtr);
	}
	...
	public synchronized void acceptCall(LinphoneCall aCall) {
		isValid();
		acceptCall(nativePtr,((Callline)aCall).nativePtr);
	...
	public synchronized Vector<LinphoneCallLog> getCallLogs() {
		...
		for (int i=0;i < getNumberOfCallLogs(nativePtr);i++) {
			logs.add(new CallLog(getCallLog(nativePtr, i)));
		}
		return logs;
	}
	...
	public synchronized LinphoneAddress interpretUrl(String destination) throws LinphoneCoreException {
		...
		if (lAddress != 0) {
			return new Address(lAddress,true);
		} else {
	...
	public synchronized LinphoneCall invite(LinphoneAddress to) throws LinphoneCoreException { 
		LinphoneCall call = (LinphoneCall)inviteAddress(nativePtr,((Address)to).nativePtr);
		if (call!=null) {
			return call;
		} else {
	...
	public synchronized void addFriend(LinphoneFriend lf) throws LinphoneCoreException {
		addFriend(nativePtr,((Friend)lf).nativePtr);
	...
	public synchronized LinphoneChatRoom createChatRoom(String to) {
		return new ChatRoom(createChatRoom(nativePtr,to));
	}
	...
	public synchronized LinphoneCallParams createDefaultCallParameters() {
		return new CallParameter(createDefaultCallParams(nativePtr));
	}
	...
	public synchronized LinphoneCall inviteAddressWithParams(LinphoneAddress to, LinphoneCallParams params) throws LinphoneCoreException {
		long ptrDestination = ((Address)to).nativePtr;
		long ptrParams =((CallParameter)params).nativePtr;
	...
	public synchronized int updateCall(LinphoneCall call, LinphoneCallParams params) {
		long ptrCall = ((Callline) call).nativePtr;
		long ptrParams = params!=null ? ((CallParameter)params).nativePtr : 0;
	...
	public synchronized LinphoneProxyConfig[] getProxyConfigList() {
		...
		for (int i=0; i < proxies.length; i++) {
			proxies[i] = new ProxyConfig(typesPtr[i]);
		}
	...
	public synchronized boolean pauseCall(LinphoneCall call) {
		return 0 == pauseCall(nativePtr, ((Callline) call).nativePtr);
	}
	public synchronized boolean resumeCall(LinphoneCall call) {
		return 0 == resumeCall(nativePtr, ((Callline) call).nativePtr);
	}
	...
	private long getCallPtr(LinphoneCall call) {
		return ((Callline)call).nativePtr;
	}
